...
flowchart LR
  ...
  subgraph EDGE["Edge-узел проходной: весь инференс и решение здесь"]
    ...
    GRD["Терминал охраны на посту: работает и без связи с центром"]
  ...
  POL -->|"manual_review: кадр из буфера и код причины"| GRD
  GRD -->|"вердикт охранника"| POL
  GRD --> JRN
  POL --> JRN --> SYN
  SYN -->|"решения и метрики, без кадров"| BUS
  SYN -->|"сводная очередь ручных проверок"| SEC
...
Кадры обычных проходов не покидают узел и не сохраняются. Кадр события `manual_review` уходит на терминал охраны той же проходной, поэтому разбор работает и при потере связи с центром. В центр отправляются решения, причины, скоры и метрики, а сводная очередь ручных проверок нужна для аналитики и для разбора постфактум.
...
sequenceDiagram
    ...
    participant GRD as Терминал охраны на посту
    ...
    alt s1 выше t_allow и margin выше delta
        ...
        EDG->>GRD: manual_review, кадр из буфера, код причины
        GRD-->>EDG: разрешить или отказать
...
Сумма верхних границ 309 мс, целимся в 500 мс p95 на попытку без повтора кадра. Ориентир задания в одну секунду относится именно к ней: если качество не годится, узел берёт следующий кадр, и попытка с тремя кадрами растягивается до трёх секунд. Поэтому SLO два: 500 мс p95 на попытку без повтора и не больше пяти процентов попыток с повтором, а секундный ориентир к попыткам с повтором не применяется. Складывать эти строки нельзя: p95 суммы не равен сумме p95 стадий, потому что стадии коррелируют через общую очередь на GPU и через качество кадра. Приёмка идёт сквозным замером от `captured_at` до отправки команды, а таблица нужна для планирования и для локализации просадки. Механика самого турникета в SLA не входит (задание меряет до команды), но её порядок называем: штанга 100–300 мс, створки 400–800 мс.
...
Пост охраны стоит на самой проходной, поэтому терминал подключён к edge-узлу напрямую и работает при потере связи с центром: иначе офлайн-сценарий разваливается, ведь именно в нём сомнительные события и появляются. Центральный интерфейс держит сводную очередь по всем проходным, аналитику и разбор постфактум, но решение по конкретному человеку принимается локально.

В карточке события: кадр из кольцевого буфера, код причины (низкое качество, серая зона PAD, малый margin, просроченный кеш, отзыв доступа), время, проходная, камера, предполагаемый сотрудник со скором. Две кнопки: разрешить проход и отказать. Вердикт пишется в audit log вместе с идентификатором охранника и уходит в набор отложенных меток для еженедельной перекалибровки порогов. Сотрудник при этом обычно не ждёт вердикта: карта остаётся быстрее, и ответ системы прямо говорит ему приложить карту, а охрана подтверждает карточку постфактум.
...
- `next_action` со значениями `open`, `use_card`, `go_to_guard`: сотруднику на табло нужна инструкция вместо кода причины, и вычислять её на стороне табло по семи полям ответа означает продублировать политику в клиенте;
